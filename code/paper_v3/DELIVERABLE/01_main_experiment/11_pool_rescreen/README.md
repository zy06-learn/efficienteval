...
The verifier pool in use was selected in paper_v1, not in part1. Its evidence is
`paper_v1/results/POOL_SCREEN_LOCKEDCFG.csv`, where
`factcc + granite_guardian_3_1_2b + lettuce_v2` ranks **1 of 540** at validation AUROC
`0.82005 @ 90.60 ms`.

That screen ran on paper_v1's 6,850-row matrix. Measured against the current splits, that
matrix contains:
...
`paper_v3/pool_rescreen_v1.py` re-runs the selection on data Protocol B's TEST never touched:
