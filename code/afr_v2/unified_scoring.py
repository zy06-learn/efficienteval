...
class BM25Top1Scorer:
    ...
    def score_batch(
        ...
        *,
        ...
        from afr_v2.router_feature_learnability import (
...
class Qwen30APIScorer:
    ...
    def score_batch(self, docs: list[str], claims: list[str]) -> list[dict[str, Any]]:
        from afr_v2.structured_high_judge import (
            SYSTEM_PROMPT,
            numbered_source,
...
def build_unified_scorer(
    ...
    *,
    ...
    if verifier in {"factcg", "minicheck_dbta", "minicheck_ft5", "alignscore", "hhem"}:
        from afr_v2.primary_scoring import build_primary_scorer
    ...
    if verifier == "lettuce_v2":
        from afr_v2.extended_scorers import LettuceV2Scorer

        return LettuceV2Scorer(device=device)
    if verifier == "fenice":
        from afr_v2.candidate_verifiers import FENICEScorer
