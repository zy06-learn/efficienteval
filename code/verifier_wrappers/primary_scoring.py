...
from verifier_wrappers.cascade_primary_assets import PRIMARY_SPLIT, SCORING_COLUMNS
...
class HHEMChunkedScorer:
    ...
    def __init__(self, *, device: str = "cuda") -> None:
        from verifier_wrappers.candidate_verifiers import HHEMScorer
...
def build_primary_scorer(action: str, *, device: str = "cuda") -> Any:
    ...
    if action == "alignscore":
        from verifier_wrappers.candidate_verifiers import AlignScorePersistentScorer

        return AlignScorePersistentScorer(device=device, batch_size=1)
    if action == "factcg":
        from verifier_wrappers.native_scorers import FactCGNativeScorer
    ...
    if action == "minicheck_dbta":
        from verifier_wrappers.minicheck_scorers import MiniCheckDebertaScorer

        return MiniCheckDebertaScorer(device=device)
    if action == "minicheck_ft5":
        from verifier_wrappers.minicheck_scorers import MiniCheckFT5Scorer
