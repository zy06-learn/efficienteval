...
Immutable source run: `experiments/runs/part1c_main_full_v1`. Wall clock 40 min 39 s, CPU only.
...
fair numbers are that search's own frozen run, which lives under `results/` in the author's
working tree and is not shipped here: `0.78118 @ 538.99 ms` in
...
`config_v2.VERIFIERS` and contains all pool members; shared's default pool equals the current
...
- **Pool provenance.** The pool was selected in an earlier round on a 6,850-row matrix that shares 54.4%
