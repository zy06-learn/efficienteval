...
Everything below was read from the source in `../../../verifiers/` during this audit. Where a claim is carried
over from the earlier notes rather than re-verified, that is stated.

## The executed path

`../../../ingest_and_scoring/p1_score.py` is the driver. It calls `build_scorer` and `score_frame` from
`../../../verifiers/unified_summary_verifiers_v1.py`. `build_scorer` dispatches each verifier to one of five
...
1. **HHEM and AlignScore are not `PairWindowScorer`s.** They are `MetadataScorer`s wrapping
   scorers built by `build_primary_scorer` in `../../../verifiers/primary_scoring.py`. HHEM's windowing is
...
HHEM uses `build_hhem_token_windows` in `../../../verifiers/primary_scoring.py` instead, which differs in two
...
Three columns in `ingest_and_scoring/results/p1_scoring/*.parquet` describe forward work, and they disagree
on purpose:
...
|---|---:|---|
| `../../../ingest_and_scoring/p1_score.py` | 115 | the driver, and how scoring outputs are written |
| `../../../verifiers/unified_summary_verifiers_v1.py` | 1342 | dispatch, windowing, four of the five wrappers, `score_frame` |
| `../../../verifiers/primary_scoring.py` | 357 | HHEM windowing, and `build_primary_scorer` for hhem / alignscore / factcg / minicheck |
| `../../../verifiers/unified_scoring.py` | 1682 | FactCC and FactKB pinned scorers |
| `../../../verifiers/extended_scorers.py` | 332 | Lettuce-v2 scorer |
| `../../../verifiers/additional_verifier_scorers_v1.py` | 449 | WeCheck scorer |
| `REGISTRY.md` | — | the verifier registry as maintained in `verifier_wrappers/` |
