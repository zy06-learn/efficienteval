...
Stages 3 and 4: the routing experiments, the controls, and every published result. This tree
is self-contained. No number in it was produced under a superseded configuration, and nothing
here depends on the round-1 and round-2 output directories, the smoke runs, or the aborted run
that exist in the author's working tree but are not part of this release.

`00_inputs/` is where to start: it carries the frozen features and verifier scores with all
source and summary text removed, and every table below is computed from it. Directory names
follow the order the stages run in; `08_routing_code/` and `09_live_and_controls/code/` hold
the code, everything numbered `01_` to `05_` holds evidence. `notes_v4.tex` is an archived
paper draft from 2026-08-17, kept for the record: its numbers were read from these archives,
but its paths refer to the working tree and are not updated.
...
The pool was selected in an earlier round on a matrix sharing 54.4% of Protocol B's test document groups,
...
Each experiment is a launcher plus a run directory. From `experiments/`:
